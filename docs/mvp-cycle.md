...
- **Domain types**: `EssayVersion` (`src/domain/essay/version.ts`) is an immutable snapshot — `EssayVersionId` (UUID v4), `essayId`, `userId`, `versionNumber`, `title`, `content` (TipTapDoc), `publishedAt`. Branded `EssayVersionId` added to `src/domain/types/branded.ts` following the existing `validateUuid<T>()` pattern.
- **Version operations** (`src/domain/essay/version-operations.ts`): `createVersionSnapshot(params)` returns `Result<EssayVersion, ValidationError>` with `versionNumber >= 1` validation (pattern consistency — every domain entity constructor returns `Result`). `diffVersions(before, after)` computes block-level diffs using LCS (Longest Common Subsequence) on canonicalized TipTap blocks. O(n*m) complexity is acceptable for micro-essays (5–20 blocks), and the client caller wraps it in `useMemo`. Post-processing via `mergeModified()` pairs adjacent removed+added blocks of the same type into `"modified"` entries for cleaner diff display.
- **Publish pipeline** (`src/domain/essay/publish-pipeline.ts`): `preparePublishWithVersion(params)` is a pure function composing `publishEssay` + `createVersionSnapshot` into a single domain step, returning `Result<PublishWithVersionResult, PublishError | ValidationError>`. `SavePublishWithVersion` port type defines the contract for the infra adapter. This keeps orchestration logic in the domain rather than the server action.
- **Atomic publish transaction** (`src/infra/essay/publish-transaction.ts`): `savePublishWithVersion` implements the `SavePublishWithVersion` port. Wraps version insert + essay upsert in a single Drizzle `db.transaction()` — if either write fails, both roll back. Essay uses `onConflictDoUpdate` with `setWhere` scoped to `userId` for safety.
- **Version repository** (`src/infra/essay/postgres-essay-version-repository.ts`): `countByEssay(essayId)` for determining the next version number, `listByEssay(essayId, userId)` ordered by `versionNumber DESC`, `findById(versionId, userId)` with ownership check. `versionToDomain` validates branded IDs at the infra boundary.
- **DB schema**: `essay_version` table with text PK (`id`), `essayId` FK (CASCADE delete), `userId` FK, `versionNumber` integer, `title`, `content` (jsonb — TipTap doc), `createdAt` (serves as `publishedAt`). Immutable rows — no update operations.
- **Server action refactoring** (`src/app/(protected)/editor/actions.ts`): `publishEssayAction` delegates to `preparePublishWithVersion` (domain) then `savePublishWithVersion` (infra), replacing direct calls to `publishEssay` and `createVersionSnapshot`. New actions: `listVersionsAction` returns version summaries with word counts, `getVersionAction` loads full version content with essay ownership verification.
- **Shared TipTap renderer** (`src/app/_shared/tiptap-renderer.tsx`): Extracted from duplicated code in `essay-renderer.tsx` and `version-block-renderer.tsx`. Pure functions (no `"use client"`): `renderBlockNode(node, key, customMark?)`, `extractBlockText(node)`. `CustomMarkRenderer` callback type allows callers to handle extension-specific marks (e.g., `evidenceAttachment`) while the shared renderer handles all base marks (bold, italic, code, strike, link) and block types. `essay-renderer.tsx` passes a custom mark handler for evidence highlights; version views pass no custom handler.
- **Version history panel** (`src/app/(protected)/editor/[id]/version-history-panel.tsx`): `"use client"` component with three view states: list (version summaries with dates, word counts, "View" and "Compare" buttons), viewing (read-only TipTap render of a historical version), and diffing (block-level diff visualization). Sticky sidebar on desktop (`lg:sticky lg:top-8`), full-width on mobile. Uses `useCallback` for handlers and `useMemo` for diff computation.
- **Diff view** (`src/app/(protected)/editor/[id]/version-diff-view.tsx`): Renders `BlockDiff[]` as color-coded blocks — green border-l for added, red border-l with strikethrough for removed, red+amber pair for modified (before/after), grey for unchanged. Uses `extractBlockText` to detect empty blocks and show `(empty block)` placeholder.
- **Editor integration** (`src/app/(protected)/editor/[id]/essay-editor.tsx`): "Version History" toolbar button toggles the version panel as a third side-panel state alongside feedback and evidence picker. Panel receives `currentDoc` (live TipTap JSON) for real-time diff comparison against historical versions.
- **Public page**: "Revised N times" badge added to the metadata line on `/essay/[id]`, driven by version count from the repository.
- **19 new tests** in `test/unit/essay/version-operations.test.ts`: `createVersionSnapshot` (4 tests — field capture, parameterized values, content fidelity, non-positive version rejection), `diffVersions` (11 tests — identical docs, additions, removals, modifications, mixed changes, empty docs, empty arrays, empty-to-content, content-to-empty, middle insertion, cross-type non-merge). Total test suite: 186 tests, <50ms.
...
- **E2E auth infrastructure**: Conditional `emailAndPassword` provider in Better Auth, gated by `isE2ETesting()` from `src/infra/env.ts` (centralized check: `E2E_TESTING === "true" && NODE_ENV !== "production"`). Google OAuth env checks relaxed in E2E mode. Test seed endpoint (`src/app/api/test/seed/route.ts`) creates users via `auth.api.signUpEmail()`, returns 404 in non-E2E mode, idempotent for existing users (detects "already", "exists", "duplicate" in error messages), returns generic error messages (no raw exception leaks).
- **Auth setup** (`test/e2e/auth.setup.ts`): Uses `page.request` (not top-level `request` fixture) so cookies from the sign-in response share the browser context's cookie jar. Credentials sourced from `E2E_TEST_EMAIL` / `E2E_TEST_PASSWORD` env vars with local-only defaults.
...
- **Total test suite**: 186 unit tests (<50ms) + 29 E2E test scenarios (8 pre-existing + 21 new).
